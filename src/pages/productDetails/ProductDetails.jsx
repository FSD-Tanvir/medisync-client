...
const ProductDetails = () => {
  ...
  const handelAddToCart = async (productDetails) => {
    ...
    if (user && user?.email) {
      ...
      try {
        await axios.post("https://medisync-server.vercel.app/productCart", cartItem);
        refetch();
        // Handle success
        Swal.fire({
          position: "center",
  ...
  return (
    <section className="mb-5">
        ...
        <div className="lg:w-1/2 border-2 border-blue-200 rounded m-2 p-4">
          ...
          <div>
            ...
            <span className="text-xl font-bold ">
              Best Price: ${productDetails?.price}{" "}
              <span className="text-blue-500">/piece</span>
            </span>
            ...
            <div className="flex justify-center items-center gap-2 border-2 p-2 mt-4 bg-[#FFF7F4] rounded">
              <button
                className="text-black text-2xl px-4 py-2"
          ...
          <button
            onClick={() => handelAddToCart(productDetails)}
            className="border hover:border-blue-500 text-white hover:text-blue-500 bg-blue-500 hover:bg-[#FFF7F4] font-semibold py-4 px-4 w-full  rounded-md text-xs sm:text-xl md:text-xs lg:text-sm xl:text-xs mt-4 shadow-[-2px_-2px_12px_2px_rgba(0,0,0,0.1),_2px_2px_12px_2px_rgba(0,0,0,0.1)]"
          >
            Add To Cart
          </button>
        </div>
      ...
      <p className="font-medium text-gray text-xl mt-2">
