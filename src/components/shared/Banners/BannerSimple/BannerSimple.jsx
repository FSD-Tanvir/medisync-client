...
const BannerSimple = ({ imgUrl, text1, text2,pageName}) => {
  return (
    <div
      className={`h-[35vh] sm:h-[55vh] w-full ${pageName === "aboutUs" ? "bg-center": "bg-center"} bg-cover bg-no-repeat`}
      style={{
        backgroundImage: `url(${imgUrl})`,
      }}
    >
      <div className={`bg-black/25 text-white h-full flex  justify-center items-center`}>
        <div className={`${pageName === "meetDoctors" && "-mt-14 "} flex`}>
        <Link to="/">
          <span className="drop-shadow-md sm:text-2xl pr-2 text-cyan-400">
            Home
          </span>
        </Link>
        <p className={`drop-shadow-md border-l-4 rounded-t-full text-center ${pageName === "career" ? "pl-2":"pl-4"}`}>
